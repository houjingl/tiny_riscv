...
module lw_tb();
    ...
    task wait_till_next_cfsm_state(input [5:0] expected_state);
        @(posedge clk); #1;
        `assert_equal(uut.M_CTRL.CTRL_FSM.fsm_current_state, expected_state)
    endtask
    ...
    initial begin
        ...
        `assert_equal(uut.instr[19:15], 2)
        `assert_equal(uut.immExt, 0)

        wait_till_next_cfsm_state(uut.M_CTRL.CTRL_FSM.MEMADR);
        ...
        wait_till_next_cfsm_state(uut.M_CTRL.CTRL_FSM.MEMREAD);
        //Verify correct mem adr was provided by ALU
        `assert_equal(uut.Result, 32'ha8)
        `assert_equal(uut.Adr, 32'ha8)

        wait_till_next_cfsm_state(uut.M_CTRL.CTRL_FSM.MEMWB);
